...
export function CartProvider({ children }) {
  ...
  const refreshCart = useCallback(
    async () => setCart(await getCurrentCart()),
    [],
  );
  useEffect(() => {
    refreshCart();
    const onVisible = () =>
      document.visibilityState === "visible" && refreshCart();
    document.addEventListener("visibilitychange", onVisible);
    return () => document.removeEventListener("visibilitychange", onVisible);
  }, [refreshCart]);

  const itemCount = (cart?.lineItems ?? []).reduce(
    (n, li) => n + (li.quantityInfo?.confirmedQuantity || 0),
    0,
  );
  ...
  const removeItem = (lineItemId) =>
    run(async () => setCart(await apiRemove(lineItemId)));
  const updateQuantity = (lineItemId, qty) =>
    run(async () => setCart(await apiQty(lineItemId, qty)));
  const checkout = () =>
    run(
      async () => {
        window.location.href = await apiCheckout();
      },
      { reread: true },
    );
