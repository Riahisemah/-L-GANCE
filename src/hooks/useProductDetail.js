...
export function useProductDetail(slug) {
  ...
  useEffect(() => {
    ...
    getProductBySlug(slug)
      .then((p) => {
        ...
        const initial = {};
        (p.options || []).forEach((o) => {
          const first = o.choicesSettings?.choices?.find(
            (c) => c.inStock !== false,
          );
      ...
      .catch((e) => {
        if (!cancelled) setError(e?.message || "Couldn't load this product.");
      });
    return () => {
      cancelled = true;
    };
  ...
  const variant = useMemo(() => {
    ...
    return (
      variants.find((v) =>
        (v.choices || []).every(
          (c) =>
            selectedOptions[c.optionChoiceIds?.optionId] ===
  ...
  const canAdd = useMemo(() => {
    ...
    return modifiers
      ...
      .every((m) =>
        m.modifierRenderType === "FREE_TEXT"
          ? !!modifierValues[m.freeTextSettings?.key]
          : !!modifierValues[m.key],
      );
  ...
  const price =
    variant?.price?.actualPrice?.formattedAmount ||
    product?.actualPriceRange?.minValue?.formattedAmount ||
    "";
  const compareAtPrice =
    variant?.price?.compareAtPrice?.formattedAmount ||
  ...
  const focusMediaUrl = useMemo(() => {
    for (const o of options) {
      const choice = (o.choicesSettings?.choices || []).find(
        (c) => c.choiceId === selectedOptions[o.id],
      );
  ...
  const setModifier = (key, value) =>
    setModifierValues((s) => ({ ...s, [key]: value }));
  ...
  async function submit() {
    ...
    modifiers.forEach((m) => {
      const k =
        m.modifierRenderType === "FREE_TEXT" ? m.freeTextSettings?.key : m.key;
      if (!k || !modifierValues[k]) return;
      (m.modifierRenderType === "FREE_TEXT"
        ? customTextFields
        : modifierChoices)[k] = modifierValues[k];
    });
    setAdding(true);
    try {
      ...
      return await addToCart(
        ...
        {
          modifierChoices: Object.keys(modifierChoices).length
            ? modifierChoices
            : undefined,
          customTextFields: Object.keys(customTextFields).length
            ? customTextFields
            : undefined,
        },
      );
