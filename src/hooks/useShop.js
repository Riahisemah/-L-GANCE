...
export function useShop({ pageSize = 24 } = {}) {
    ...
    queryCategories({ limit: 100 })
      .then(({ categories: list }) => {
        if (alive)
          setCategories(
            (list || []).filter(
              (c) => c.visible !== false && c.slug !== SYSTEM_CATEGORY_SLUG,
            ),
          );
      })
      .catch(() => {
      ...
      .catch((e) => {
        if (alive && generation.current === id && currentKey.current === key) {
          setPage({
            ...
            products: [],
            cursor: null,
            error: e?.message || "Couldn't load products.",
            loadingMore: false,
          });
        }
      });
  ...
  const loadMore = useCallback(async () => {
    ...
    const isCurrent = () =>
      generation.current === request.generation && currentKey.current === key;
    ...
    try {
      ...
      if (isCurrent())
        setPage((p) => ({
          ...p,
          error: e?.message || "Couldn't load more products.",
        }));
    } finally {
  ...
  return {
    ...
    filters,
    setFilters,
    products: current ? page.products : null,
