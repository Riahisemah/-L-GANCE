...
export function useProductCard(product) {
  return useMemo(() => {
    ...
    const isPreorder =
      isSoldOut && product?.inventory?.preorderStatus === "ENABLED";
    ...
    if (isPreorder) leftBadges.push({ type: "pre-order", label: "Pre-order" });
      ...
      leftBadges.push({ type: "sold-out", label: "Sold out" });
    if (isPartiallyOutOfStock)
      leftBadges.push({ type: "limited-stock", label: "Limited stock" });
    ...
    const compareAtDisplay =
      compareAt && compareAt !== priceMin ? compareAt : null;
    ...
    for (const o of product?.options || []) {
      const choices = (o.choicesSettings?.choices || []).filter(
        (c) => c.visible !== false,
      );
      ...
      labels.push(
        `${choices.length} ${choices.length === 1 || n.endsWith("s") ? n : `${n}s`}`,
      );
    }
