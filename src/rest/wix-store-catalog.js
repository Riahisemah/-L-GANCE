...
export async function searchProducts({
  ...
  if (!cursor) {
    if (!Object.hasOwn(CATALOG_SORTS, sort))
      throw new Error("Unsupported catalog sort.");
    const min = priceBound(minPrice, "minPrice"),
      max = priceBound(maxPrice, "maxPrice");
    ...
    if (min !== undefined)
      conditions.push({
        "actualPriceRange.minValue.amount": { $gte: String(min) },
      });
    if (max !== undefined)
      conditions.push({
        "actualPriceRange.minValue.amount": { $lte: String(max) },
      });
    if (inStockOnly)
      conditions.push({ "inventory.availabilityStatus": { $eq: "IN_STOCK" } });
    query = {
      ...query,
      filter: { $and: conditions },
      ...(SORT_FIELDS[sort] ? { sort: SORT_FIELDS[sort] } : {}),
      ...(search.trim()
        ? { search: { expression: search.trim(), fields: ["name"] } }
        : {}),
    };
  }
...
export async function getProductBySlug(slug) {
  const res = await wixApiRequest(
    `/stores/v3/products/slug/${encodeURIComponent(slug)}`,
    {
      method: "GET",
...
export async function getCategoryBySlug(slug) {
  const res = await wixApiRequest(
    `/categories/v1/categories/slug/${encodeURIComponent(slug)}`,
    {
      method: "GET",
