...
export async function addToCart(
  ...
  if (modifierChoices && Object.keys(modifierChoices).length)
    catalogReferenceOptions.options = modifierChoices;
  if (customTextFields && Object.keys(customTextFields).length)
    catalogReferenceOptions.customTextFields = customTextFields;

  const catalogReference = { appId: STORES_APP_ID, catalogItemId };
  if (Object.keys(catalogReferenceOptions).length)
    catalogReference.options = catalogReferenceOptions;
  ...
  const line = (res?.cart?.lineItems ?? []).find(
    (l) =>
      l.source?.catalogReference?.catalogItemId === catalogItemId &&
      (!variantId ||
        l.source?.catalogReference?.options?.variantId === variantId),
  );
  ...
  if (line?.status && line.status !== "IN_STOCK") {
    throw new Error(
      `Item not available for sale (status: ${line.status}). Is it in stock?`,
    );
  }
...
export async function getCurrentCart() {
  try {
    const res = await wixApiRequest("/ecom/v2/carts/current", {
      method: "GET",
    });
...
export async function checkout() {
  ...
  if (unavailable.length) {
    const names = unavailable
      .map((l) => l.name?.original ?? l.source?.catalogReference?.catalogItemId)
      .join(", ");
    throw new Error(
      `Cannot check out: ${unavailable.length} item(s) not available — ${names}.`,
    );
  }
  if (!cart.id)
    throw new Error("Failed to check out: the current cart has no id.");

  const redirect = await wixApiRequest("/headless/v1/redirect-session", {
    method: "POST",
    body: {
      ecomCheckout: { checkoutId: cart.id },
