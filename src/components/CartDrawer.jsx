...
export default function CartDrawer() {
  ...
  const unavailable = lineItems.filter(
    (li) => li.status && li.status !== "IN_STOCK",
  );
      ...
      >
        <header className="flex justify-between items-center p-4 border-b border-border">
          <strong className="font-display">
            Your cart{lineItems.length ? ` (${lineItems.length})` : ""}
          </strong>
          <button
            onClick={() => setIsOpen(false)}
            aria-label="Close cart"
            className="border-none bg-transparent cursor-pointer text-xl text-muted-foreground"
          >
            ×
          </button>
        </header>
        ...
        {error && (
          <div
            role="alert"
            className="m-4 mb-0 p-3 rounded-sm border border-destructive/40 bg-destructive/10 text-sm"
          >
            <div className="flex items-start gap-2">
              ...
              <button
                onClick={clearError}
                aria-label="Dismiss"
                className="border-none bg-transparent cursor-pointer text-muted-foreground leading-none"
              >
                ×
              </button>
            </div>
          ...
          {lineItems.length === 0 ? (
            ...
            lineItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex gap-3 py-3 px-0 border-b border-border"
                >
                  ...
                  <div className="flex-1 flex flex-col gap-1 min-w-0">
                    ...
                    {item.attributes?.descriptionLines?.map((dl, i) => (
                      <small key={i} className="text-muted-foreground">
                        {dl.name?.original}:{" "}
                        {dl.plainText?.original || dl.colorInfo?.original}
                      </small>
                    ))}
                    {isOut ? (
                      <small className="text-destructive">
                        {item.status === "PARTIALLY_IN_STOCK"
                          ? "Not enough stock"
                          : "No longer available"}
                      </small>
                    ) : atStockLimit ? (
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <button
                        ...
                        disabled={loading}
                        className="ml-auto border-none bg-transparent cursor-pointer text-muted-foreground text-[13px] underline disabled:opacity-50"
                      >
                        Remove
                      </button>
                    </div>
...
function QtyButton({ children, onClick, disabled, label }) {
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="w-7 h-7 cursor-pointer leading-none bg-card text-foreground border border-border rounded-sm disabled:opacity-40 disabled:cursor-not-allowed"
    >
      {children}
    </button>
  );
